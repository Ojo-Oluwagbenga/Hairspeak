...
// JavaScript Document
        ...
        // const fileUrls = JSON.parse(document.getElementById("file-urls").textContent);
        // console.log(fileUrls);
        // Coverflow functionality
        const hair_records = JSON.parse($("meta[name='hair_records']").attr("content"));
        console.log(hair_records);
        let text = ``;
        let imageData = [
        ];
        for (let i = 0; i < hair_records.file_urls.length; i++) {
            const url = hair_records.file_urls[i];
            imageData.push({
                title:hair_records.filenames[i],
                description:""
            })
            text += `<div class="coverflow-item" data-index="0">
                        <div class="cover image-loading">
                            <img src="${url}" alt="Mountain Landscape" loading="lazy">
                        </div>
